...
get_print(Nth, Reg, Acc, [Nth-'#'|Acc]) :-
    B is Reg - 1,
    E is Reg + 1,
    Nth2 is (Nth - 1) mod 40,
    between(B, E, Nth2), !.
get_print(Nth, _, Acc, [Nth-'.'|Acc]).

solution2 :-
    read-input(input, I),
    loop(0, I, 1, get_print, [], R),
    foreach( ( member(B, [1,41,81,121,161,201]),
               E is B + 39 ),
             ( foreach( ( between(B, E, X),
                          member(X-S, R) ),
                        write(S) ),
               nl )
           ).
%% (By query `?- solution2.` and see the print,)
