...
occupied_size(Z) :-
    findall(S, path_size(["/"|_], S), L),
    sum_list(L, Z).

solution2(S) :-
    Full = 70_000_000,
    Lower_bound = 30_000_000,
    read-input(input),
    occupied_size(O),
    Size_to_delete is Lower_bound - Full + O,
    findall( X,
             ( dir(Folder),
               total_size(Folder, X),
               Size_to_delete =< X
             ...
             Sizes),
    smallest(Sizes, Lower_bound, S).
%% (By query `?- solution2(Size)`)
%% Your puzzle answer was `Size`.
